...
def assign_to_me_comm(assign_after):
	...
	while True:
		...
		if incidents_numbers == 0:
			...
			if updated[0].text.startswith('j') or updated[0].text.startswith('a'):
				...
				if((updated_time >= int(assign_after)) or (AE_Counter >= 10)):
...
def assign_to_me_comm_init(username,password,url,assign_after):
	...
	while True:
		try:
			...
			assign_to_me_comm(assign_after)
...
def assign_to_me_comm_muti(username,password,url,assign_after):
	th = threading.Thread(target=assign_to_me_comm_init,args=(username,password,url,assign_after))  
...
Label(root, font=('Consolas',12),fg = 'white',bg='grey', width=17, height=2, text="SIMS ID").grid(row=1,sticky = W + E + N + S,pady=2)
Label(root, font=('Consolas',12),fg = 'white',bg='grey', width=17, height=2, text="Password").grid(row=2,sticky = W + E + N + S)
Label(root, font=('Consolas',12),fg = 'white',bg='grey', width=17, height=2, text="ServiceNow URL").grid(row=3,sticky = W + E + N + S,pady=2)
Label(root, font=('Consolas',12),fg = 'white',bg='grey', width=17, height=2, text="Assign After").grid(row=4, sticky = W + E + N + S)
Label(root, font=('Consolas',12),fg = 'white',bg='grey', width=17, height=2, text="Assign Every").grid(row=5, sticky = W + E + N + S,pady=2)
...
e2 = Entry(root, show='*', font=('Consolas',12),fg = 'grey',width=20, textvariable = y)
...
e5 = Entry(root, font=('Consolas',12),fg = 'grey',width=20, textvariable = m)
...
e5.grid(row=5, column=1,sticky = W + E + N + S,pady=2)

button_start_comm = Button(root,text = "Start as Commodity",relief=FLAT, font=('Consolas',12),fg = 'white',bg='LightSlateGray',command = lambda:assign_to_me_comm_muti(e1.get(),e2.get(),e3.get(),e4.get()))
button_start_fast = Button(root,text = "Start as Fast",relief=FLAT, font=('Consolas',12),fg = 'white',bg='LightSlateGray')
button_start_all = Button(root,text = "Start as All",relief=FLAT, font=('Consolas',12),fg = 'white',bg='LightSlateGray')
button_start_comm.grid(row = 16, column = 0, rowspan = 60, columnspan = 2, sticky = W + E + N + S)
button_start_fast.grid(row = 78, column = 0, rowspan = 60, columnspan = 2, sticky = W + E + N + S)
button_start_all.grid(row = 140, column = 0, rowspan = 60, columnspan = 2, sticky = W + E + N + S)
